...
def run_command(command, capture_output=True, text=True):
    ...
    result = subprocess.run(
        command, capture_output=capture_output, text=text, shell=True
    )
...
def main():
    ...
    version = (
        input(f"Enter the version number [{default_version}]: ").strip()
        or default_version
    )

    # Collect commits since the last tag
    commits = run_command(
        f"git log {latest_tag}..HEAD --pretty=format:'%h %s'",
        capture_output=True,
        text=True,
    )
    ...
    run_command(
        f'git tag -a {version} -m "{tag_message}"', capture_output=False, text=False
    )
