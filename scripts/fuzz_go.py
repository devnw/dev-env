...
def main():
    ...
    try:
        ...
        files = (
            run_command(
                "grep -r --include='**_test.go' --files-with-matches 'func Fuzz' ."
            )
            .strip()
            .split("\n")
        )
        ...
        for file in files:
            ...
            funcs = (
                run_command(f"grep -o 'func Fuzz\\w*' {file} | sed 's/func //'")
                .strip()
                .split("\n")
            )
            ...
            for func in funcs:
                ...
                try:
                    subprocess.check_call(
                        f"go test {parent_dir} -run='^{func}$' -fuzz='^{func}$' -fuzztime={fuzz_time}s",
                        shell=True,
                    )
